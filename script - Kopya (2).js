...
const movies = {
  ...
  theamazingspiderman: {
    ...
    desc: "Peter Parker faces the dangerous Lizard."
  },
...
const movieLinks = { spiderverse: "https://www.fullhdfilmizlesene.life/film/orumcek-adam-orumcek-evrenine-gecis-fh1/",
  ...
  hababamsinifi: "https://www.youtube.com/watch?v=hf2-8MRPGu8" }; // SENİN LİNKLER AYNI KALDI
...
function openMovie(movieId) {
    ...
    document.getElementById("watchBtn").onclick = () => playMovie(movieId);
}
...
const quizData = {
    ...
    theamazingspiderman: [
        ...
        { q: "Dr. Connors'ın sürüngen DNA'sı üzerinde çalışmasının asıl kişisel sebebi nedir?", a: ["Kanser tedavisi", "Eksik kolunu geri çıkartmak", "Ölümsüzlük arayışı", "Askeri silah üretmek"], correct: 1 },
    ...
    thegodfather: [
        ...
        { q: "Michael Corleone, babasına suikast düzenleyen Sollozzo ve polis şefini nerede vurarak mafya dünyasına adım atar?", a: ["Bir kumarhanede", "İtalyan restoranında", "Kendi evinde", "Sokak ortasında"], correct: 1 },
        { q: "Corleone ailesinin hukuki işlerine bakan, aileden sayılan evlatlık danışmanın (Consigliere) adı nedir?", a: ["Tom Hagen", "Peter Clemenza", "Salvatore Tessio", "Luca Brasi"], correct: 0 },
        ...
        { q: "Don Vito Corleone'nin dünyaca ünlü o meşhur repliği hangisidir?", a: ["Para her kapıyı açar", "Ona reddedemeyeceği bir teklif yapacağım", "Dostunu yakın tut, düşmanını daha yakın", "Adalet mahkemede değil sokaktadır"], correct: 1 }
    ],
    esaretinbedeli: [
        ...
        { q: "Andy Dufresne'in hapishanede tanıştığı ve her şeyi temin edebilen en yakın dostu kimdir?", a: ["Red (Morgan Freeman)", "Brooks", "Tommy", "Heywood"], correct: 0 },
        ...
        { q: "Andy, kaçış planını tamamladıktan sonra Red'e buluşmaları için işaret ettiği Meksika'daki o sahil kasabasının adı nedir?", a: ["Zihuatanejo", "Cancun", "Tijuana", "Acapulco"], correct: 0 },
...
function startQuiz(movieKey) {
    currentQuestions = quizData[movieKey];
    if (!currentQuestions) return alert("Bu film için henüz quiz hazırlanmadı!");
    
    currentQuestionIndex = 0;
    score = 0;
    
    document.getElementById("quizTitle").innerText = movieKey.toUpperCase() + " QUIZ";
    document.getElementById("questionContainer").style.display = "block";
    document.getElementById("resultContainer").style.display = "none";
    
    // Paneli ekranda göster (Senin CSS'indeki mantıkla sola kaydırıyoruz)
...
function showQuestion() {
    ...
    optionsDiv.innerHTML = ""; // Eski seçenekleri temizle
    
    qData.a.forEach((option, index) => {
        const btn = document.createElement("button");
        btn.innerText = option;
        // Senin CSS'indeki buton tasarımına benzer şık butonlar oluşturuyoruz
...
function logoutUser() {
    // Küçük bir onay kutusu gösterelim (İsteğe bağlı, direkt de attırabilirsin)
    if (confirm("CinePru oturumunu kapatmak istediğine emin misin?")) {
        // Eğer giriş bilgilerini localStorage veya sessionStorage üzerinde tutuyorsan temizle:
        // localStorage.removeItem("isLoggedIn"); 
        
        // Kullanıcıyı login.html sayfasına yönlendiriyoruz
